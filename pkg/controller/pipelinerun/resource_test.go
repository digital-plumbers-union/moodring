...
func TestFindGitResourceWithRepository(t *testing.T) {
	...
	want := &resource.PipelineResourceSpec{
		Type: "git",
...
func TestgetRepoAndSHAFromResource(t *testing.T) {
	...
	for _, tt := range resourceTests {
		...
		if !matchError(t, tt.wantErr, err) {
			t.Errorf("getRepoAndSHAFromResource() %s: got error %v, want %s", tt.name, err, tt.wantErr)
			continue
		}

		if tt.repo != repo {
			t.Errorf("getRepoAndSHAFromResource() %s: got repo %s, want %s", tt.name, repo, tt.repo)
		}

		if tt.sha != sha {
			t.Errorf("getRepoAndSHAFromResource() %s: got SHA %s, want %s", tt.name, sha, tt.sha)
		}
	}
}
...
func makePipelineRunWithResources(opts ...tb.PipelineRunSpecOp) *pipelinev1.PipelineRun {
	return tb.PipelineRun(pipelineRunName, tb.PipelineRunSpec(
		"tomatoes", opts...,
	), tb.PipelineRunNamespace(testNamespace), tb.PipelineRunStatus(tb.PipelineRunStatusCondition(
...
func makePipelineResource(resType pipelinev1.PipelineResourceType, url, rev string) *resource.PipelineResourceSpec {
